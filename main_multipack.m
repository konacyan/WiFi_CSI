...
L = 3;
L_s = L;
sample_csi_trace = reshape(csi(1,:,:),N_r,N_c).'; 
T = 1;
...
%     csi_sanitization = squeeze(sample_csi_trace(i_t,:,:));
    csi_sanitization = sample_csi_trace;
...
% Thetahat_tensor
% Thetahat_spotFi
Theta_result_tensor(1:L,i_csi) = Thetahat_tensor(1:L);
Theta_result_spotFi(1:L_s,i_csi) = Thetahat_spotFi(1:L_s);
end
save(['Result/',filename,'_',num2str(pack_num),'.mat'],'Theta_result_tensor','Theta_result_spotFi','angle_true');
