...
int main(int argc, char** argv)
{
	...
	uint32_t mode;
	uint32_t delay_us;
	struct timespec start, now, timestamp;
	int32_t diff;
...
*/
	...
	//add 20210227,fixed parameter 
	delay_us = 10;
	mode = 1; 
	packet_size = 100;
	num_packets = 1000;
	printf("--------------------------------------------------");
	printf("\nDelay us: %d, Packet Size:%d, Packet Num:%d\n",delay_us, packet_size,num_packets);
	...
	if (delay_us) {
		...
		clock_gettime(CLOCK_REALTIME, &timestamp);
    		printf("Start Sending: CLOCK_REALTIME： %ld sec，%ld nanosec\n", timestamp.tv_sec, timestamp.tv_nsec);
	...
	for (i = 0; i < num_packets; ++i) {
		...
		if (((i+1) % 500) == 0) {
			printf(".");
			clock_gettime(CLOCK_REALTIME, &timestamp);
    		printf("%d Data: CLOCK_REALTIME： %ld sec，%ld nanosec\n", (i+1), timestamp.tv_sec, timestamp.tv_nsec);
