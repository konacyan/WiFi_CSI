...
angle_true =  180/pi*atan(groundtruth_y/groundtruth_x)
...
%     end
    ...
    for i_t = 1:N_t
            subplot(1,2,i_t)
            plot(subcarrier_idx,csi_unwrapphase(:,(i_t - 1)*N_r + 1),'r*-');
            hold on;
            plot(subcarrier_idx,csi_unwrapphase(:,(i_t - 1)*N_r + 2),'bo-');
            hold on;
            plot(subcarrier_idx,csi_unwrapphase(:,(i_t - 1)*N_r + 3),'k+-');  
            hold off;
            xlabel('Subcarrier Index','FontName','Times New Roman','FontSize',14);
            ylabel('Unwrap Phase','FontName','Times New Roman','FontSize',14);
            legend1 = legend('RX 1','RX 2','RX 3');
    ...
    for i_t = 1:N_t
            subplot(1,2,i_t)
            plot(csi_Difference(:,(i_t - 1)*N_r + 1),'r*-');
            hold on;
            plot(csi_Difference(:,(i_t - 1)*N_r + 2),'bo-');
            hold on;
            plot(csi_Difference(:,(i_t - 1)*N_r + 3),'k+-');  
            hold off;
            xlabel('Subcarrier Index');
            ylabel('Unwrap Phase');
            legend('RX 1 - RX 2','RX 1 - RX 3','RX 2 - RX 3');
    end
%     csi_Difference(:,1)= csi_unwrapphase(:,2) - csi_unwrapphase(:,1);
%     csi_Difference(:,2)= csi_unwrapphase(:,3) - csi_unwrapphase(:,2);  
%     csi_Difference(:,3)= csi_unwrapphase(:,5) - csi_unwrapphase(:,4);
%     csi_Difference(:,4)= csi_unwrapphase(:,6) - csi_unwrapphase(:,5);

    csi_difference(:,1)= abs(csi_Difference(:,2)) - abs(csi_Difference(:,1));
    csi_difference(:,2)= abs(csi_Difference(:,4)) - abs(csi_Difference(:,3));
